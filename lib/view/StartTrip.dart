      ...
      if (user != null) {
        try {
          ...
          for (File photo in localPhotos) {
            ...
            Reference ref =
                FirebaseStorage.instance.ref().child('trip_images/$fileName.jpg');
          ...
          final trip = {
            ...
            'photoPaths': uploadedPhotoUrls.join(", "),
            'userId': user.uid,
          ...
          _showSnack("Error posting trip: $e");
        } finally {
    ...
    switch (key) {
      ...
      case "boardingPoint":
        if (formData[key].toString().isEmpty) {
          _showSnack("Please select ${key == "destination" ? "destination" : "boarding city"}");
          return false;
        }
        break;
      case "groupSize":
      ...
      case "budget":
        if (formData["minBudget"].toString().isEmpty ||
          ...
          _showSnack("Please enter both min & max budget");
          return false;
        }
        final min = double.tryParse(formData["minBudget"].toString()) ?? 0;
        final max = double.tryParse(formData["maxBudget"].toString()) ?? 0;
        if (max <= min) {
          _showSnack("Max budget must be greater than min budget");
          return false;
        }
        break;
      case "details":
        if ((formData["details"].toString().trim().split(RegExp(r'\s+')).length) <
            30) {
          _showSnack("Please enter at least 30 words");
          return false;
        }
        break;
      case "photos":
    ...
    return Column(
      children: [
        TextField(
          ...
          onChanged: (val) =>
              PlaceSearchController.onChange(val, isDest ? "destination" : "boarding"),
                    ...
                    return Theme(
                      data: ThemeData.light().copyWith(
                        colorScheme: const ColorScheme.light(
                          primary: Colors.teal, // ✅ Green calendar theme
        ...
        children: [
          Expanded(
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  Text("Min", style: GoogleFonts.poppins(fontSize: 14, fontWeight: FontWeight.w500)),
                  const SizedBox(height: 6),
                  buildTextInput("minBudget", Icons.currency_rupee),
                ],
              ),
            ),
          ),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text("Max", style: GoogleFonts.poppins(fontSize: 14, fontWeight: FontWeight.w500)),
                const SizedBox(height: 6),
                buildTextInput("maxBudget", Icons.currency_rupee),
              ],
            ),
          ),
        ],
  ...
  Widget buildPhotoPicker() => Column(
        children: [
          ElevatedButton.icon(
            ...
            label: const Text("Pick Photo(s)"),
            style: ElevatedButton.styleFrom(backgroundColor: Colors.teal),
            onPressed: () async {
              final List<XFile>? images = await _picker.pickMultiImage();
              if (images != null && images.isNotEmpty) {
                setState(() {
                  final currentPhotos = formData["photos"] as List<File>;
                  final remainingSlots = 4 - currentPhotos.length;
                  final selected = images.take(remainingSlots);
                  currentPhotos.addAll(selected.map((x) => File(x.path)));
                });
              }
            },
          ),
